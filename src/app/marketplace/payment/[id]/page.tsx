...
export default function PaymentPage() {
  ...
  const { data: order, loading: orderLoading, error: orderError } = useOrderDetail(orderId);
  const { confirm: confirmPayment, loading: paymentLoading, error: paymentError } = useConfirmPayment();
...
const handlePaymentSubmit = async () => {
  ...
  try {
    ...
    console.log(`Simulating payment for Order ID: ${orderId} via ${selectedPaymentMethod}`);
    ...
    toast.error("Payment failed", {
      description: paymentError || err.message  || "An unknown error occurred.",
    });
  }
};
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 max-w-3xl mx-auto px-4 py-10 pt-24 space-y-8 bg-white shadow-lg rounded-xl my-8">
        ...
        <p className="text-lg text-gray-600 text-center">
          Your order is confirmed by the farmer. Please proceed with payment to finalize your purchase.
        </p>
        ...
        <div className="border border-gray-200 rounded-lg p-6 space-y-4 bg-blue-50">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-gray-700">
            <p><span className="font-medium">Product:</span> {order.product_name || 'N/A'}</p>
            <p><span className="font-medium">Quantity:</span> {order.quantity}</p>
            <p><span className="font-medium">Farmer:</span> {order.farmer_name || 'N/A'}</p>
            <p><span className="font-medium">Delivery:</span> {order.delivery_days ? getDeliveryDateRange(order.delivery_days) : 'N/A'}</p>
        ...
        <div className="border border-gray-200 rounded-lg p-6 space-y-4">
          ...
          <div className="space-y-3">
            <label className="flex items-center space-x-3 cursor-pointer p-3 border rounded-md hover:bg-gray-50 transition-colors">
              ...
              />
              <span className="text-lg font-medium text-gray-700">Mobile Money (MTN, Vodafone, AirtelTigo)</span>
            </label>
            <label className="flex items-center space-x-3 cursor-pointer p-3 border rounded-md hover:bg-gray-50 transition-colors">
              <input
                type="radio"
                ...
                className="form-radio text-green-600 h-5 w-5"
              />
              <span className="text-lg font-medium text-gray-700">Bank Transfer</span>
            </label>
            <label className="flex items-center space-x-3 cursor-pointer p-3 border rounded-md hover:bg-gray-50 transition-colors">
              <input
                type="radio"
          ...
          {selectedPaymentMethod && selectedPaymentMethod !== 'cash_on_delivery' && (
            <div className="space-y-2 mt-4">
              <label htmlFor="transactionRef" className="block text-sm font-medium text-gray-700">
              ...
              <p className="text-xs text-gray-500">
                Please enter the reference number provided by your payment service.
              </p>
            </div>
          )}
          ...
          <Button
            ...
            disabled={paymentLoading || !selectedPaymentMethod || (selectedPaymentMethod !== 'cash_on_delivery' && !transactionRef)}
            size="lg"
